...
public class Manager{  // Manager 클래스 -> 게임 진행 상태를 판단
    public static boolean compare(int[] compareComputer, int[] comparePlayer) {
        ...
        for(int i = 0; i < compareComputer.length; i++) {
            if(compareComputer[i] == comparePlayer[i]) {    //스트라이크 체크
                strike++;
            }
            for(int j = 0; j < compareComputer.length; j++) {    //볼 체크
                if(compareComputer[i]== comparePlayer[j] && i!=j) {
                    ball++;
                    break;
                }
            }
        ...
        return strike == compareComputer.length;
    }
}